...
RSpec.describe Api::V1::Revenues::MostItemsController, type: :controller do
  it "Gets top items by total revenue" do
    item1 = Item.create(name: "Wrench")
    item2 = Item.create(name: "Hammer")
    iteminv1 = InvoiceItem.new(unit_price: 20000, quantity: 2, item: item1)
    iteminv2 = InvoiceItem.new(unit_price: 10000, quantity: 1, item: item2)
    ...
    expect(response.body).to eq  "[{\"id\":#{item1.id},\"name\":\"Wrench\"}]"
  end
